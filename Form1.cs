...
namespace Gobang
{
    public partial class Form1 : Form
    {
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            {
                string moveSymbol = isPlayer1Turn ? "●" : "○";
                clickedButton.Text = moveSymbol; // 玩家1使用實心圈，玩家2使用空心圈
                clickedButton.Font = new Font(clickedButton.Font.FontFamily, 14, isPlayer1Turn ? FontStyle.Bold : FontStyle.Regular);
                clickedButton.Enabled = false;

                moveCount++;
                UpdateMoveHistory($"Move {moveCount}: {moveSymbol}");
        ...
        private int CountConsecutiveSymbols(int row, int col, int rowIncrement, int colIncrement, string symbol)
        {
            ...
            while (row >= 0 && row < Size && col >= 0 && col < Size && buttons[row, col].Text == symbol)
            {
                count++;
                row += rowIncrement;
                col += colIncrement;
